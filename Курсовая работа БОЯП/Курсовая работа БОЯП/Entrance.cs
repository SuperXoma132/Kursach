...
namespace Курсовая_работа_БОЯП
{
    public partial class Entrance : Form
    {
        ...
        private void authButton_Click(object sender, EventArgs e) //авторизация
        {
            ...
            {
                SqlCommand usersCountCommand = new SqlCommand($"SELECT COUNT(*) FROM Users WHERE Login = N'{authLoginBox.Text}' AND Password = N'{authPasswordBox.Text}'", sqlConnection1);
                object usersCount = usersCountCommand.ExecuteScalar();
                if (Convert.ToInt32(usersCount.ToString()) == 1)
                {
                    SqlCommand userRole = new SqlCommand($"SELECT Role FROM Users WHERE Login = N'{authLoginBox.Text}' AND Password = N'{authPasswordBox.Text}'", sqlConnection1);
                    CurrentUserData.UserRole = userRole.ExecuteScalar().ToString().Trim();
                    SqlCommand userId = new SqlCommand($"SELECT Id FROM Users WHERE Login = N'{authLoginBox.Text}' AND Password = N'{authPasswordBox.Text}'", sqlConnection1);
                    CurrentUserData.UserId = userId.ExecuteScalar().ToString().Trim();
                    SqlCommand userLogin = new SqlCommand($"SELECT Login FROM Users WHERE Login = N'{authLoginBox.Text}' AND Password = N'{authPasswordBox.Text}'", sqlConnection1);
        ...
        private void regButton_Click(object sender, EventArgs e) //регистрация
        {
            ...
            {
                SqlCommand usersCountCommand = new SqlCommand($"SELECT COUNT(*) FROM Users WHERE Login = N'{regLoginBox.Text}'", sqlConnection1);
                object usersCount = usersCountCommand.ExecuteScalar();
                if (Convert.ToInt32(usersCount.ToString()) == 0)
                {
                    ...
                    else
                    {
                        ...
                        SqlCommand addUserToDB = new SqlCommand($"INSERT INTO Users (Id, Login, Password, Role) VALUES (N'{id}', N'{regLoginBox.Text}', N'{regPasswordBox.Text}', N'Гость')", sqlConnection1);
        ...
        {
            sqlConnection1.Open();
            SqlCommand deleteStudents = new SqlCommand($"DELETE FROM Students", sqlConnection1);
            deleteStudents.ExecuteNonQuery();
            SqlCommand deleteUsers = new SqlCommand($"DELETE FROM Users", sqlConnection1);
